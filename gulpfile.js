...
function browsersync() {
  connect.server(
    ...
    function () {
      browserSync.init({
        // Use the init() method instead of directly calling the function
        proxy: "127.0.0.1:8000", // Proxy to the PHP server
        port: 8080,
        open: true,
        notify: false,
      });
...
function startwatch() {
  ...
  watch(["./*.php"]).on("change", function () {
    browserSync.reload(); // Reload when PHP files change
  });
}
